...
public class editusercontroller extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
       ...
       String doj=request.getParameter("udoj");
    	   ...
    	   if(role==3)
    	   {
    	   userd.updateUser(user_id, user_name, email, date_joined, level, layer, mobile_no, manager_id_1, manager_id_2,panelist_flag,password);
    	   rd=request.getRequestDispatcher("editusers.jsp");
    	   }
    	   else
    	   {
    		   userd.updateAdmin(user_id, user_name, email, date_joined, level, layer, mobile_no,password);
