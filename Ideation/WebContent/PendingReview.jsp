...
<div class="container">
  ...
  <c:set var="uid" value="${sessionScope.user.user_id }"/>
  <c:set var="iuid" value="${sessionScope.idea.user_id }"/>
  <c:if test="${uid ne iuid }">
  	<h4>Idea By: ${sessionScope.idea.user_id}</h4>
  </c:if><br/><br/>
  <strong>Title: </strong>
  ...
  <sql:query dataSource="${myidea}" var="result" >
	...
	<br/>
  <c:choose>
  <c:when test="${uid eq iuid }">
  <div class="jumbotron">
    <h4>Idea yet to be reviewed by panel.</h4>
  </div>
  </c:when>
  <c:otherwise>
	<sql:query dataSource="${myidea}" var="result" >
